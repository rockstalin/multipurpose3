...
router.route('/home')
    .get(function(req, res) {
         ...
         Parse.Cloud.run('fetchUser', {objectId:req.session.objectId,appId:req.session.app.objectId}).then(function(object) {
            if (object && object.get("type") === "admin" && (object.get("creatorAccess") === true || object.get("adminAccess") === true || object.get("superAdminAccess") === true || object.get("editorAccess") === true)) {
                user = object

                if(object.get("adminAccess") === true || object.get("adminAccess") === true || object.get("superAdminAccess") === true ){
            ...
            if(httpResponse){
                ...
                //res.send(JSON.stringify(req.session.objectId, null, 4));//edit

                return Parse.Cloud.run('app', {adminId:req.session.objectId}) // Correct
            }
            ...
            if(objects){
                ...
                req.session.creatorAccess = user.get("creatorAccess")
                ...
                if(user.get("creatorAccess") === true || user.get("adminAccess") === true || user.get("superAdminAccess") === true || user.get("editorAccess") === true){
                    var ejs = 'index_editor' 
                    if(user.get("creatorAccess") === true ||user.get("adminAccess") === true || user.get("superAdminAccess") === true){
                        ejs = 'index'
                    }
                    res.render(ejs, {
                        title: cons.DEVELOPER_NAME_BOLD,
                        user:user,
                        creatorAccess:req.session.creatorAccess,
...
//console.log("'/installations'",req.session.params)
...
router.route('/app_list')
    .get(function(req, res) {
        if(level(1,req)){
         ...
         //res.send(JSON.stringify(req.session, null, 4)); //edit

         //var params = {"objectId":"req.session.app.objectId"}

          //req.session.app.objectId =

         renderEjs(req,res,"list_controller","Applications",undefined,{adminId:req.session.objectId},"app","app.ejs",false,undefined)
        }else{
            res.status(403).send()
        }
    });    
router.route('/add_app')
    ...
    .get(function(req, res) {
        if(level(1,req)){
            renderEjs(req,res,"add_app","Add Application",undefined,{objectId:req.session.app.objectId},undefined,undefined,false,undefined)
        }else{
            res.status(403).send()
        }
    });
        ...
        if(level(1,req)){
            Parse.Cloud.httpRequest({
              ...
              body: {}
                    ...
                    //console.log(".......category ", httpResponse.data.result)

                var categories =httpResponse.data.result
                if(categories.length === 0){
                    res.redirect('add_category')
                }else{
                    //res.send(JSON.stringify(req.session, null, 4));
                    renderEjs(req,res,"add_post","Add Post",undefined,undefined,undefined,undefined,false,categories)

                    /*
                    Parse.Cloud.run('app', {objectId:req.session.app.objectId,adminId:req.session.objectId}).then(function(app){
...
router.route('/loadData')
    .post(function(req, res) {
        if(level(2,req)){
          //console.log("/loadData" ,req.body, req.session.app,user.id,req.session.objectId)
