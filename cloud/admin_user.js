...
Parse.Cloud.define("addDefaultAdmin", function(request, response) {
    ...
    query.count().then(function(count) {
        if(count<1){
            ...
            object.set("creatorAccess", creatorAccess)
            object.set("adminAccess", adminAccess)
            object.set("editorAccess", editorAccess)
            

            //object.set("addedBy",object.get("objectId"))
                       
//                       response.success(object.get("objectId"))
//
//                       response.success(object.get("objectId"))
//
...
function save(objectUser,response,file){
    ...
    objectUser.save().then(function(objectUser) {
        ...
        object.set("companyName", "Gemmy Apps")
                           
                           //objectUser.set("addedBy","creator")

                          // console.log(+objectUser.id)

                           response.success("save .........already added!!" + objectUser.get("objectId"),objectUser.id)

                           
                                          object.set("addedBy", objectUser)
        ...
        object.save().then(function(){
            ...
            objectSys.set("value", true)
                           ...
                           superAdmin.id = objectUser.id;
                           objectUser.set("addedBy", superAdmin)
                           
                           objectUser.save(null,{useMasterKey:true}).then(function(object){
                                                                      response.success(object)
                                                                      },function(error){
                                                                      response.error(error)
                                                                      })

                          // response.error("stalin 1 " + object)
...
Parse.Cloud.define("addAdmin", function(request, response) {
   ...
   }else if (creatorAccess === undefined) {
        response.error("please supply the creatorAccess")
...
function updateUser(object,objectArchive,superAdminId,response){
    ...
    //response.error(".........updateAdmin" +object)
